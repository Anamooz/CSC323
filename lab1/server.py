...
urls = ("/", "index", "/forgot", "forgot", "/register", "register", "/reset", "reset")
...
class index:
    myform = form.Form(
        form.Textbox(
            "username", form.notnull, description="Username", id="usernameBox"
        ),
        form.Password(
            "password", form.notnull, description="Password", id="passwordBox"
        ),
        form.Button("Login", id="loginButton"),
    )
...
class forgot:
    myform = form.Form(
        form.Textbox("user", form.notnull, description="Username", id="forgotUser"),
        form.Button("Reset", description="Send"),
        id="forgotButton",
    )
...
class register:
    myform = form.Form(
        ...
        form.Button("Register", description="Register"),
    )
    ...
    def POST(self):
        ...
        if not form.validates():
            ...
            if form.d.user in user_dic:
                ...
                user_dic[form.d.user] = hashlib.sha1(
                    form.d.password.encode("UTF-8")
                ).hexdigest()
                msg = "User registered."
...
class reset:
    myform = form.Form(
        ...
        form.Hidden("token", form.notnull, value="", description="Reset Token"),
    ...
    def GET(self):
        ...
        myform = form.Form(
            form.Password("password", form.notnull, description="New Password"),
            form.Hidden("token", form.notnull, value=token, description="Reset Token"),
            form.Button("Reset Password", description="Register"),
        )
        msg = ""
        err = ""
